# -*- coding: utf-8 -*-
"""
Created on Tue Oct 15 22:32:55 2019
...
print(check_output(['ls', '../input']).decode('utf-8')) ### need pl

train = pd.read_csv(r'E:\python_projects\Store Item Demand Forecasting Challenge-kaggle\train.csv')
...
first_diff = train_df.sales - train_df.sales.shift(1) ## Calculating I value by shify=ting one value
first_diff = first_diff.dropna(inplace = False) # first NaN value we are removing 
...
## In our case, we see the first order differencing make the ts stationary. I = 1.
...
## Since seasonality is still theere we need to consider SARIMAX model
...
holiday= pd.read_csv(r'E:\python_projects\Store Item Demand Forecasting Challenge-kaggle/USholidays.csv')
...
sarimax_mod.forecast(steps = 91,exog = exog_data[start_index:end_index])


train_df['forecast'] = sarimax_mod.predict(start = pd.to_datetime(start_index), end= pd.to_datetime(end_index1),
